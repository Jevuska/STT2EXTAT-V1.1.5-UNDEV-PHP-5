...
function _stt2extat_data_format( $data, $html, $count, $number, $display, $convert, $widget )
{
	...
	if ( array_filter( $list ) && 0 < $number ) :
		$list = wp_list_pluck( array_slice( $list, 0, $number ), 'text_link' );
...
/**
 * disable auto update
 * filter to auto_update_plugin
 *
 * @since 1.1.5-undev
 *
*/
function stt2extat_auto_update_plugin( $update, $item )
{
    if ( in_array( $item->slug, array( 'stt2extat', ) ) )
		return false;
}

/**
 * disable notification update
 * filter to site_transient_update_plugins
 *
 * @since 1.1.5-undev
 *
*/
function stt2extat_disabled_notice_update( $value )
{
	if ( isset( $value->response[ STT2EXTAT_PLUGIN_BASENAME ] ) )
		unset( $value->response[ STT2EXTAT_PLUGIN_BASENAME ] );
	return $value;
}